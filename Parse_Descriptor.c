 ...
 *
 ******************************************/
int GetNetworkNameDescriptor(NETWORK_NAME_DESCRIPTOR_T *pstNetworkNameDescriptor, unsigned char *pucDescriptorBuffer, int iDescriptorBufferLength)
{
	int iDescriptorPosition = 0;
	memset(pstNetworkNameDescriptor, 0, sizeof(NETWORK_NAME_DESCRIPTOR_T));
	
	iDescriptorPosition = GetDescriptorTagPosition(iDescriptorPosition, pucDescriptorBuffer, iDescriptorBufferLength, NETWORK_NAME_DESCRIPTOR_TAG);
 ...
 *
 ******************************************/
int GetServiceListDescriptor(SERVICE_LIST_DESCRIPTOR_T *pstServiceListDescriptor, unsigned char *pucDescriptorBuffer, int iDescriptorBufferLength)
{
	int iDescriptorPosition = 0;
	...
	iDescriptorPosition = GetDescriptorTagPosition(iDescriptorPosition, pucDescriptorBuffer, iDescriptorBufferLength, SERVICE_LIST_DESCRIPTOR_TAG);
	if (-1 == iDescriptorPosition)
	{
		printf("There is not 'ServiceListDescriptor' in descriptor buffer\n");
		return -1;
	}
	else
	{
 ...
 *
 **********************************************************/
int GetCableDeliverySystemDescriptor(CABLE_DELIVERY_SYSTEM_DESCRIPTOR_T *pstCableDeliverySystemDescriptor, unsigned char *pucDescriptorBuffer, int iDescriptorBufferLength)
{
	int iDescriptorPosition = 0;
	memset(pstCableDeliverySystemDescriptor, 0, sizeof(SERVICE_LIST_DESCRIPTOR_T));
	iDescriptorPosition = GetDescriptorTagPosition(iDescriptorPosition, pucDescriptorBuffer, iDescriptorBufferLength, CABLE_DELIVERY_SYSTEM_DESCRIPTOR_TAG);
	
	if (-1 == iDescriptorPosition)
	{
		printf("There is not 'CableDeliverySystemDescriptor' in descriptor buffer\n");
		return -1;
	}
	else
	{
 ...
 *
 **********************************************************/
int GetSatelliteDeliverySystemDescriptor(SATELLITE_DELIVERY_SYSTEM_DESCRIPTOR_T *pstSatelliteDeliverySystemDescriptor, unsigned char *pucDescriptorBuffer, int iDescriptorBufferLength)
{
	int iDescriptorPosition = 0;
	memset(pstSatelliteDeliverySystemDescriptor, 0, sizeof(SATELLITE_DELIVERY_SYSTEM_DESCRIPTOR_T));
	iDescriptorPosition = GetDescriptorTagPosition(iDescriptorPosition, pucDescriptorBuffer, iDescriptorBufferLength, SATELLITE_DELIVERY_SYSTEM_DESCRIPTOR_TAG);
	
	if (-1 == iDescriptorPosition)
	{
		printf("There is not 'SatelliteDeliverySystemDescriptor' in descriptor buffer\n");
		return -1;
	}
	else
	{
 ...
 *
 ******************************************/
int GetBouquetNameDescriptor(BOUQUET_NAME_DESCRIPTOR_T *pstBouquetNameDescriptor, unsigned char *pucDescriptorBuffer, int iDescriptorBufferLength)
{
	int iDescriptorPosition = 0;
	memset(pstBouquetNameDescriptor, 0, sizeof(BOUQUET_NAME_DESCRIPTOR_T));
	iDescriptorPosition = GetDescriptorTagPosition(iDescriptorPosition, pucDescriptorBuffer, iDescriptorBufferLength, BOUQUET_NAME_DESCRIPTOR_TAG);
	if (-1 == iDescriptorPosition)
	{
		return -1;
	}
 ...
 *
 ******************************************/
int GetServiceDescriptor(SERVICE_DESCRIPTOR_T *pstServiceDescriptor, unsigned char *pucDescriptorBuffer, int iDescriptorBufferLength)
{
	int iDescriptorPosition = 0;
	memset(pstServiceDescriptor, 0, sizeof(SERVICE_DESCRIPTOR_T));
	iDescriptorPosition = GetDescriptorTagPosition(iDescriptorPosition, pucDescriptorBuffer, iDescriptorBufferLength, SERVICE_DESCRIPTOR_TAG);
	if (-1 == iDescriptorPosition)
	{
		return -1;
	}
 ...
 *
 ******************************************/
int GetLinkageDescriptor(LINKAGE_DESCRIPTOR_T *pstLinkageDescriptor, unsigned char *pucDescriptorBuffer, int iDescriptorBufferLength)
{
	int iDescriptorPosition = 0;
	int iLengthBeforePrivateData = 0;
	memset(pstLinkageDescriptor, 0, sizeof(LINKAGE_DESCRIPTOR_T));
	iDescriptorPosition = GetDescriptorTagPosition(iDescriptorPosition, pucDescriptorBuffer, iDescriptorBufferLength, LINKAGE_DESCRIPTOR_TAG);
	if (-1 == iDescriptorPosition)
	{
		return -1;
	}
 ...
 *
 ******************************************/
int GetStreamIndentifierDescriptor(STREAM_IDENTIFIER_DESCRIPTOR_T *pstStreamIndentifierDescriptor, unsigned char *pucDescriptorBuffer, int iDescriptorBufferLength)
{
	int iDescriptorPosition = 0;
	memset(pstStreamIndentifierDescriptor, 0, sizeof(STREAM_IDENTIFIER_DESCRIPTOR_T));
	iDescriptorPosition = GetDescriptorTagPosition(iDescriptorPosition, pucDescriptorBuffer, iDescriptorBufferLength, STREAM_IDENTIFIER_DESCRIPTOR_TAG);
	if (-1 == iDescriptorPosition)
	{
		return -1;
	}
 ...
 *
 ******************************************/
int GetTeletextDescriptor(TELETEXT_DESCRIPTOR_T *pstTeletextDescriptor, unsigned char *pucDescriptorBuffer, int iDescriptorBufferLength)
{
	int iDescriptorPosition = 0;
	int iLoopCount = 0;
	int iDescriptorInfoPostion = 0;
	memset(pstTeletextDescriptor, 0, sizeof(TELETEXT_DESCRIPTOR_T));
	iDescriptorPosition = GetDescriptorTagPosition(iDescriptorPosition, pucDescriptorBuffer, iDescriptorBufferLength, TELETEXT_DESCRIPTOR_TAG);
	if (-1 == iDescriptorPosition)
	{
		return -1;
	}
 ...
 *
 ******************************************/
int GetSubtitlingDescriptor(SUBTITLING_DESCRIPTOR_T *pstSubtitlingDescriptor, unsigned char *pucDescriptorBuffer, int iDescriptorBufferLength)
{
	int iDescriptorPosition = 0;
	int iLoopCount = 0;
	memset(pstSubtitlingDescriptor, 0, sizeof(TELETEXT_DESCRIPTOR_T));
	iDescriptorPosition = GetDescriptorTagPosition(iDescriptorPosition, pucDescriptorBuffer, iDescriptorBufferLength, SUBTITLING_DESCRIPTOR_TAG);
	if (-1 == iDescriptorPosition)
	{
		return -1;
	}
	else
	{
		...
		{
			pstSubtitlingDescriptor->astSubtitling_Info[iLoopCount].stISO_639_Language_code.aucPrivate_data_byte[0] = pucDescriptorBuffer[2 + iDescriptorPosition + iLoopCount * 8];
			pstSubtitlingDescriptor->astSubtitling_Info[iLoopCount].stISO_639_Language_code.aucPrivate_data_byte[1] = pucDescriptorBuffer[3 + iDescriptorPosition + iLoopCount * 8];
			pstSubtitlingDescriptor->astSubtitling_Info[iLoopCount].stISO_639_Language_code.aucPrivate_data_byte[2] = pucDescriptorBuffer[4 + iDescriptorPosition + iLoopCount * 8];
 ...
 *
 **********************************************************/
int GetTerrestrialDeliverySystemDescriptor(TERRESTRIAL_DELIVERY_SYSTEM_DESCRIPTOR_T *pstTerrestrialDeliverySystemDescriptor, unsigned char *pucDescriptorBuffer, int iDescriptorBufferLength)
{
	int iDescriptorPosition = 0;
	memset(pstTerrestrialDeliverySystemDescriptor, 0, sizeof(TERRESTRIAL_DELIVERY_SYSTEM_DESCRIPTOR_T));
	iDescriptorPosition = GetDescriptorTagPosition(iDescriptorPosition, pucDescriptorBuffer, iDescriptorBufferLength, TERRESTRIAL_DELIVERY_SYSTEM_DESCRIPTOR_TAG);
	
	if (-1 == iDescriptorPosition)
	{
		printf("There is not 'TerrestrialDeliverySystemDescriptor' in descriptor buffer\n");
		return -1;
	}
	else
	{
 ...
 *
 ******************************************/
int GetShortEventDescriptor(SHORT_EVENT_DESCRIPTOR_T *pstShortEventDescriptor, unsigned char *pucDescriptorBuffer, int iDescriptorBufferLength)
{
	int iDescriptorPosition = 0;
	memset(pstShortEventDescriptor, 0, sizeof(SHORT_EVENT_DESCRIPTOR_T));
	iDescriptorPosition = GetDescriptorTagPosition(iDescriptorPosition, pucDescriptorBuffer, iDescriptorBufferLength, SHORT_EVENT_DESCRIPTOR_TAG);
	if (-1 == iDescriptorPosition)
	{
		return -1;
	}
 ...
 *
 ******************************************/
int GetExtendedEventDescriptor(EXTENDED_EVENT_DESCRIPTOR_T *pstExtendedEventDescriptor, unsigned char *pucDescriptorBuffer, int iDescriptorBufferLength)
{
	int iDescriptorPosition = 0;
	int iLoopCount = 0;
	int iDescriptorInfoPostion = 0;
	memset(pstExtendedEventDescriptor, 0, sizeof(EXTENDED_EVENT_DESCRIPTOR_T));
	iDescriptorPosition = GetDescriptorTagPosition(iDescriptorPosition, pucDescriptorBuffer, iDescriptorBufferLength, EXTENDED_EVENT_DESCRIPTOR_TAG);
	if (-1 == iDescriptorPosition)
	{
		return -1;
	}
 ...
 *
 *********************************************************/
int GetFrequencyListDescriptor(FREQUENCY_LIST_DESCRIPTOR_T *pstFrequencyListDescriptor, unsigned char *pucDescriptorBuffer, int iDescriptorBufferLength)
{
	int iDescriptorPosition = 0;
	int iLoopCount = 0;
	memset(pstFrequencyListDescriptor, 0, sizeof(FREQUENCY_LIST_DESCRIPTOR_T));
	iDescriptorPosition = GetDescriptorTagPosition(iDescriptorPosition, pucDescriptorBuffer, iDescriptorBufferLength, FREQUENCY_LIST_DESCRIPTOR_TAG);
	if (-1 == iDescriptorPosition)
	{
		return -1;
	}
	else
	{
		...
		{
			pstFrequencyListDescriptor->astCentre_frequency[iLoopCount].uiCentre_frequency = (pucDescriptorBuffer[3 + iDescriptorPosition + iLoopCount * 4] << 24) | (pucDescriptorBuffer[4 + iDescriptorPosition + iLoopCount * 4] << 16)
					| (pucDescriptorBuffer[5 + iDescriptorPosition + iLoopCount * 4] << 8) | (pucDescriptorBuffer[6 + iDescriptorPosition + iLoopCount * 4]);
		}
		return iDescriptorPosition;
	}
}
 ...
 *
 ******************************************/
int GetVideoStreamDescriptor(VIDEO_STREAM_DESCRIPTOR_T *pstVideoStreamDescriptor, unsigned char *pucDescriptorBuffer, int iDescriptorBufferLength)
{
	int iDescriptorPosition = 0;
	memset(pstVideoStreamDescriptor, 0, sizeof(VIDEO_STREAM_DESCRIPTOR_T));
	iDescriptorPosition = GetDescriptorTagPosition(iDescriptorPosition, pucDescriptorBuffer, iDescriptorBufferLength, VIDEO_STREAM_DESCRIPTOR_TAG);
	if (-1 == iDescriptorPosition)
	{
		return -1;
	}
...
int GetAudioStreamDescriptor(AUDIO_STREAM_DESCRIPTOR_T *pstAudioStreamDescriptor, unsigned char *pucDescriptorBuffer, int iDescriptorBufferLength, int iDescriptorPosition)
{
	...
	iDescriptorPosition = GetDescriptorTagPosition(iDescriptorPosition, pucDescriptorBuffer, iDescriptorBufferLength, AUDIO_STREAM_DESCRIPTOR_TAG);
	if (-1 == iDescriptorPosition)
	{
		return -1;
	}
 ...
 *
 ******************************************/
int GetDataStreamAlignmentDescriptor(DATA_STREAM_ALIGNMENT_DESCRIPTOR_T *pstDataStreamAlignmentDescriptor, unsigned char *pucDescriptorBuffer, int iDescriptorBufferLength)
{
	int iDescriptorPosition = 0;
	memset(pstDataStreamAlignmentDescriptor, 0, sizeof(DATA_STREAM_ALIGNMENT_DESCRIPTOR_T));
	iDescriptorPosition = GetDescriptorTagPosition(iDescriptorPosition, pucDescriptorBuffer, iDescriptorBufferLength, DATA_STREAM_ALIGNMENT_DESCRIPTOR_TAG);
	if (-1 == iDescriptorPosition)
	{
		return -1;
	}
 ...
 *
 ******************************************/
int GetMaximumBitrateDescriptor(MAXIMUM_BITRATE_DESCRIPTOR_T *pstMaximumBitrateDescriptor, unsigned char *pucDescriptorBuffer, int iDescriptorBufferLength)
{
	int iDescriptorPosition = 0;
	memset(pstMaximumBitrateDescriptor, 0, sizeof(MAXIMUM_BITRATE_DESCRIPTOR_T));
	iDescriptorPosition = GetDescriptorTagPosition(iDescriptorPosition, pucDescriptorBuffer, iDescriptorBufferLength, MAXIMUM_BITRATE_DESCRIPTOR_TAG);
	if (-1 == iDescriptorPosition)
	{
		return -1;
	}
...
int GetCA_Descriptor(CA_DESCRIPTOR_T *pstCA_Descriptor, unsigned char *pucDescriptorBuffer, int iDescriptorBufferLength, int iDescriptorPosition)
{
	...
	iDescriptorPosition = GetDescriptorTagPosition(iDescriptorPosition, pucDescriptorBuffer, iDescriptorBufferLength, CA_DESCRIPTOR_TAG);
	if (-1 == iDescriptorPosition)
	{
		return -1;
	}
 ...
 *
 ******************************************/
int GetSystemClockDescriptor(SYSTEM_CLOCK_DESCRIPTOR_T *pstSystemClockDescriptor, unsigned char *pucDescriptorBuffer, int iDescriptorBufferLength)
{
	int iDescriptorPosition = 0;
	memset(pstSystemClockDescriptor, 0, sizeof(SYSTEM_CLOCK_DESCRIPTOR_T));
	iDescriptorPosition = GetDescriptorTagPosition(iDescriptorPosition, pucDescriptorBuffer, iDescriptorBufferLength, SYSTEM_CLOCK_DESCRIPTOR_TAG);
	if (-1 == iDescriptorPosition)
	{
		return -1;
	}
 ...
 *
 ******************************************/
int GetISO_639_Language_Descriptor(ISO_639_LANGUAGE_DESCRIPTOR_T *pstISO_639_LanguageDescriptor, unsigned char *pucDescriptorBuffer, int iDescriptorBufferLength)
{
	int iDescriptorPosition = 0;
	int iDescriptorLength = 0;
	int iDescriptorLoopCount = 0;
	memset(pstISO_639_LanguageDescriptor, 0, sizeof(ISO_639_LANGUAGE_DESCRIPTOR_T));
	iDescriptorPosition = GetDescriptorTagPosition(iDescriptorPosition, pucDescriptorBuffer, iDescriptorBufferLength, ISO_639_LANGUAGE_DESCRIPTOR_TAG);
	if (-1 == iDescriptorPosition)
	{
		return -1;
	}
